...
void VideoFaceDetector::detectFaceAllSizes(const cv::Mat &frame)
{
    ...
    m_faceCascade->detectMultiScale(frame, m_allFaces, 1.12, 3, 0,
...
void VideoFaceDetector::detectFaceAroundRoi(const cv::Mat &frame)
{
    ...
    m_faceCascade->detectMultiScale(frame(m_faceRoi), m_allFaces, 1.12, 3, 0,
       cv::Size(m_trackedFace.width * 9 / 10, m_trackedFace.height * 9 / 10),
       cv::Size(m_trackedFace.width * 11 / 10, m_trackedFace.width * 11 / 10));
