...
//    imwrite(name,img);
    ...
    for ( int stage = 0; stage < global_params.max_numstage; stage++){     // 对于每一阶
        struct feature_node ** binfeatures ;
        binfeatures = DeriveBinaryFeat(RandomForest_[stage],images,current_shapes,bounding_boxs);  // 随机森林得到lbf
        GlobalPrediction(binfeatures, current_shapes,bounding_boxs,stage);   // 全局预测得到大的LBF
        ReleaseFeatureSpace(binfeatures, images.size());  
