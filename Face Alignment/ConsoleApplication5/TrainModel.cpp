...
void TrainModel(vector<string> trainDataName){
    ...
    for(int i=0;i<trainDataName.size();i++){
       ...
       // LoadData(path, images, ground_truth_shapes, bounding_boxs);
          LoadOpencvBbxData(path, images, ground_truth_shapes, bounding_boxs);   // 加载图片
    }

    LBFRegressor regressor;
    regressor.Train(images,ground_truth_shapes,bounding_boxs);  // 训练模型
    regressor.Save(modelPath+"LBF.model");  // 保存为LBF.model
    return;
}
