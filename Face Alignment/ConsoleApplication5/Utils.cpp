...
void LoadOpencvBbxData(string filepath,
    ...
    Mat gray;
    // --Detection
    cascade.load(cascadeName);   // 图片检测，加载分类器
    string name;
    while(getline(fin,name)){
        name.erase(0, name.find_first_not_of(" \t"));
        name.erase(name.find_last_not_of(" \t") + 1);
        ...
        // --Detection
        cascade.detectMultiScale( smallImg, faces,     // 对每一张图片进行检测
                                 1.1, 2, 0
        ...
        for( vector<Rect>::const_iterator r = faces.begin(); r != faces.end(); r++){
            Rect rect = *r;
            if (IsShapeInRect(ground_truth_shape,rect,scale)){   // 保存好检测的图片、真实图片以及检测到人脸的矩形框
                Point center;
