...
*/
	...
	if (camera.isOpened() && image.empty())   // 视频流，不是图像
	{
		...
		for (;;)
		{
			...
			detectAndDraw(frame1, cascade, regressor, scale, tryflip); // 检测并标记
...
void detectAndDraw( Mat& img, CascadeClassifier& cascade,
    ...
    for( vector<Rect>::const_iterator r = faces.begin(); r != faces.end(); r++, i++ ){
        ...
        t =(double)cvGetTickCount();
        Mat_<double> current_shape = regressor.Predict(gray,boundingbox,1);   // 预测形状
        t = (double)cvGetTickCount() - t;
